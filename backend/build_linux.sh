...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE}  FaceRec AI Worker Binary Builder for Linux${NC}"
echo -e "${BLUE}====================================================${NC}"
echo ""

# Function to show usage
show_usage() {
    echo "Usage: ./build_linux.sh [cpu|gpu|openvino|rocm]"
    echo ""
    echo "Options:"
    echo "  cpu       Build with standard CPU execution provider (default)"
    echo "  gpu       Build with NVIDIA CUDA execution provider"
    echo "  openvino  Build with Intel OpenVINO execution provider"
    echo "  rocm      Build with AMD ROCm execution provider"
    echo ""
}
...
case "$TARGET" in
    cpu)
        ...
        ;;
    gpu)
        ONNX_PACKAGE="onnxruntime-gpu>=1.19.0"
        SUFFIX="CUDA"
        ;;
    openvino)
        ...
        ;;
    rocm)
        ONNX_PACKAGE="onnxruntime-rocm>=1.19.0"
        SUFFIX="ROCm"
        ;;
    help|--help|-h)
        show_usage
...
if ! command -v python3 &> /dev/null; then
    ...
    echo "Please install Python 3.9+ (e.g. sudo apt install python3 python3-venv python3-pip)"
    exit 1
fi
...
# 4. Compile GUI application to Linux Executable
BINARY_NAME="FaceRec_AI_Worker_Linux_${SUFFIX}"
...
rm -rf "dist/${BINARY_NAME}" "build/${BINARY_NAME}"

# Use ":" as the directory separator on macOS/Linux and add --collect-binaries
pyinstaller --noconfirm --noconsole --onefile --name="${BINARY_NAME}" --add-data "app:app" --add-data "facerec.proto:." --collect-binaries onnxruntime ai_worker_gui.py
